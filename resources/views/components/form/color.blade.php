...
<div class="flex flex-col gap-1.5" x-data="{
    ...
    <div class="relative">
        ...
        >
            <span class="w-5 h-5 rounded-md border border-black/10 shrink-0 shadow-xs"
                  :style="'background:' + selected"></span>
            <span class="flex-1 text-gray-700 font-mono text-xs" x-text="selected.toUpperCase()"></span>
        ...
        <div
            ...
            class="absolute z-50 mt-2 w-full bg-white border border-gray-100 rounded-2xl shadow-lg p-4 flex flex-col gap-4"
            style="display:none"
        >
            <div class="grid grid-cols-6 gap-2">
                <template x-for="color in swatches" :key="color">
                    <button
                        ...
                        :class="selected === color ? 'ring-2 ring-offset-1 ring-indigo-400 scale-110' : 'hover:scale-110'"
                        class="w-8 h-8 rounded-lg border border-black/10 shadow-xs transition-transform duration-100"
                    ></button>
                </template>
            </div>
            ...
            <div class="flex items-center gap-3">
                <div class="relative w-10 h-10 shrink-0">
                    <span class="absolute inset-0 rounded-xl border border-black/10 shadow-xs"
                ...
                <div class="flex flex-col gap-0.5">
                    <span class="text-xs font-medium text-gray-600">Custom color</span>
                    <span class="text-xs text-gray-400 font-mono" x-text="custom.toUpperCase()"></span>
                </div>
            </div>
        </div>
    </div>
